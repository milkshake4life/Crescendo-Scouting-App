...
eventKey = "2024cave" #ONLY CHANGE THIS VARIABLE
...
for i in range(0, len(blueTeams)):
    for j in range(0, 3):
        firebase.put('https://crescendo-scouting-app-default-rtdb.firebaseio.com/Ventura/teams/' + str(blueTeams[i][j].replace("frc", "")) + '/Match-Info/' + matchNum[i], 'None', 'None')
        firebase.put('https://crescendo-scouting-app-default-rtdb.firebaseio.com/Ventura/teams/' + str(redTeams[i][j].replace("frc", "")) + '/Match-Info/' + matchNum[i], 'None', 'None')
